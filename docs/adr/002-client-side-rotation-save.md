...
## 背景
- MVPで要求される操作（表示・回転・保存）がすべてブラウザ上で完結するUXを提供したい。
...
- OCR向きレコメンドは選択ページがあればそのページのみ、未選択なら全ページをPNG化して `/api/ocr/orientation` に送信し、回転マップへ自動適用する。
...
## 2025-12-19 追記（UI更新）
- プレビューはサムネイル一覧（4-5列）で一覧し、クリック/ドラッグで複数選択、ダブルクリックで拡大表示する。
- 回転は選択ページに対して Ctrl/Cmd+矢印（90/180度）で実行し、Escで選択解除する。

## 2025-12-23 追記（プレビュー領域のドラッグ&ドロップ）
- 拡大プレビュー表示中でもPDFをドロップできるようにし、ファイル再読み込みの動線を一貫させる。
