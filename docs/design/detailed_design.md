...
- OCR補助：複数ページ推定中の進捗表示、手動中止/再開、連続回転（高尤度の同方向判定に挟まれたページを一括回転）。
...
3) ページ選択：クリック/ドラッグで複数選択、ダブルクリックで拡大プレビュー。  
   - 拡大プレビュー表示中もドラッグ&ドロップでPDFを再読み込みできる。  
...
- OCR操作：複数ページ処理中は「処理中止」を表示し、中止した場合のみ「処理再開」を有効化。完了時は完了メッセージを表示。
...
## 12. 運用・監視
- ログローテーション: 現状は未対応。必要になったら winston の日次ローテーションを追加する。
