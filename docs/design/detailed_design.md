...
5) OCRレコメンド：現在選択ページを起点に順番に画像化→/api/ocr/orientation 呼び出し→推定角度と信頼度をUIに表示し、推定結果を回転マップに自動反映。  
...
- PDF.js設定
  ...
  - サムネイルは `renderPageToCanvas` で小さく描画し、回転はPDF.js viewportに反映する。
...
- パフォーマンス
  - 初期表示：サムネイル一覧を順次描画。サムネイルサイズを小さく保つことで描画負荷を抑える。
  - プレビューは必要時のみレンダリングする。
